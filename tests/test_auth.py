"""Test for auth methods supported by Cloudwave 8"""

import os
import pycloudwave

# pycloudwave.connections.DEBUG = True
# pycloudwave._auth.DEBUG = True
...
def test_sha256_no_password():
    con = pycloudwave.connect(user="nopass_sha256", host=host, port=port, ssl=None)
    con.close()


def test_sha256_no_passowrd_ssl():
    con = pycloudwave.connect(user="nopass_sha256", host=host, port=port, ssl=ssl)
    con.close()


def test_sha256_password():
    con = pycloudwave.connect(
...
def test_caching_sha2_no_password():
    con = pycloudwave.connect(user="nopass_caching_sha2", host=host, port=port, ssl=None)
    con.close()


def test_caching_sha2_no_password_ssl():
    con = pycloudwave.connect(user="nopass_caching_sha2", host=host, port=port, ssl=ssl)
    con.close()
