...
* Removed ``escape_dict``, ``escape_sequence``, and ``escape_string`` from ``pycloudwave``
  module. They are still in ``pycloudwave.converters``.
...
* Add `"_binary" prefix` to string literal for binary types.
  ...
  connection encoding.
* `pycloudwave.Binary()` returns `bytearray` on Python 2.  This is required to distinguish
  binary and string.
* Auth plugin support.
...
* Refactoring for some more compatibility with MySQLdb including a fake
  pycloudwave.version_info attribute.
