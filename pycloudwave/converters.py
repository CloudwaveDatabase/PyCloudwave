...
def escape_float(value, mapping=None):
    s = repr(value)
    if s in ("inf", "-inf", "nan"):
        raise ProgrammingError("%s can not be used with Cloudwave" % s)
    if "e" not in s:
        #s += "e0"  # 禁止将浮点数转为科学表示法
        pass
    return s
...
def convert_timedelta(obj):
    ...
    Note that Cloudwave always returns TIME columns as (+|-)HH:MM:SS, but
...
def convert_time(obj):
    ...
    Also note that Cloudwave's TIME column corresponds more closely to
...
#
#     the snippet above is right, but CloudwaveDB doesn't process bits,
...
# for CloudwaveDB compatibility
conversions = encoders.copy()
conversions.update(decoders)
Thing2Literal = escape_str

# Run doctests with `pytest --doctest-modules pycloudwave/converters.py`
