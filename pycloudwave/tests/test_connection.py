...
from pycloudwave.constants import CLIENT
...
class TempUser:
    def __init__(self, c, user, db, auth=None, authdata=None, password=None):
        ...
        try:
            c.execute(create)
            self._created = True
        except pycloudwave.err.InternalError:
...
class TestAuthentication(base.PyCloudwaveTestCase):
    socket_auth = False
    socket_found = False
    two_questions_found = False
    three_attempts_found = False
    pam_found = False
    cloudwave_old_password_found = False
    ...
    db = base.PyCloudwaveTestCase.databases[0].copy()
    ...
    cur = pycloudwave.connect(**db).cursor()
    del db["user"]
    ...
    for r in cur:
        ...
        if r[3] == "auth_socket.so" or r[0] == "unix_socket":
            ...
            if pam_plugin_name == "auth_pam":
                pam_plugin_name = "pam"
            # Cloudwave: authentication_pam
            # https://dev.cloudwave.com/doc/refman/5.5/en/pam-authentication-plugin.html

            # MariaDB: pam
            # https://mariadb.com/kb/en/mariadb/pam-authentication-plugin/

            # Names differ but functionality is close
        elif r[0] == "cloudwave_old_password":
            cloudwave_old_password_found = True
    ...
    def test_plugin(self):
        ...
        cur = conn.cursor()
        cur.execute(
            "select plugin from cloudwave.user where concat(user, '@', host)=current_user()"
        )
        for r in cur:
            self.assertIn(conn._auth_plugin_name, (r[0], "cloudwave_native_password"))
    ...
    def realtestSocketAuth(self):
        with TempUser(
            ...
            c = pycloudwave.connect(user=TestAuthentication.osuser, **self.db)
    ...
    def realTestDialogAuthTwoQuestions(self):
        ...
        with TempUser(
            self.connect().cursor(),
            "pycloudwave_2q@localhost",
            self.databases[0]["database"],
            "two_questions",
            "notverysecret",
        ) as u:
            with self.assertRaises(pycloudwave.err.OperationalError):
                pycloudwave.connect(user="pycloudwave_2q", **self.db)
            pycloudwave.connect(
                user="pycloudwave_2q",
    ...
    def realTestDialogAuthThreeAttempts(self):
        ...
        with TempUser(
            ...
            with self.assertRaises(pycloudwave.err.OperationalError):
                pycloudwave.connect(
                    user="pycloudwave_3a", auth_plugin_map={b"dialog": object}, **self.db
                )
    ...
    def realTestPamAuth(self):
        ...
        except pycloudwave.OperationalError as e:
        ...
        with TempUser(
            ...
            try:
                c = pycloudwave.connect(user=TestAuthentication.osuser, **db)
                db["password"] = "very bad guess at password"
                with self.assertRaises(pycloudwave.err.OperationalError):
                    pycloudwave.connect(
                        user=TestAuthentication.osuser,
                        auth_plugin_map={
                            b"cloudwave_cleartext_password": TestAuthentication.DefectiveHandler
                        },
                        **self.db,
                    )
            except pycloudwave.OperationalError as e:
    ...
    def testAuthSHA256(self):
        ...
        with TempUser(
            c,
            "pycloudwave_sha256@localhost",
            self.databases[0]["database"],
            "sha256_password",
        ) as u:
            c.execute("SET PASSWORD FOR 'pycloudwave_sha256'@'localhost' ='Sh@256Pa33'")
            c.execute("FLUSH PRIVILEGES")
            db = self.db.copy()
            db["password"] = "Sh@256Pa33"
            # Although SHA256 is supported, need the configuration of public key of the cloudwave server. Currently will get error by this test.
            with self.assertRaises(pycloudwave.err.OperationalError):
                pycloudwave.connect(user="pycloudwave_sha256", **db)
    ...
    def testAuthEd25519(self):
        ...
        with TempUser(
            ...
            pycloudwave.connect(user="pycloudwave_ed25519", password="", **db)
        ...
        with TempUser(
            ...
            pycloudwave.connect(user="pycloudwave_ed25519", password="ed25519_password", **db)


class TestConnection(base.PyCloudwaveTestCase):
    def test_utf8mb4(self):
        """This test requires Cloudwave >= 3.1"""
        arg = self.databases[0].copy()
        arg["charset"] = "utf8mb4"
        conn = pycloudwave.connect(**arg)
    ...
    def test_connection_gone_away(self):
        """
        http://dev.cloudwave.com/doc/refman/5.0/en/gone-away.html
        http://dev.cloudwave.com/doc/refman/5.0/en/error-messages-client.html#error_cr_server_gone_error
        """
        con = self.connect()
        cur = con.cursor()
        cur.execute("SET wait_timeout=1")
        time.sleep(2)
        with self.assertRaises(pycloudwave.OperationalError) as cm:
    ...
    def test_init_command(self):
        ...
        conn.close()
        with self.assertRaises(pycloudwave.err.Error):
    ...
    def test_defer_connect(self):
        ...
        c = pycloudwave.connect(defer_connect=True, **d)
    ...
    def test_ssl_connect(self):
        ...
        with mock.patch(
            "pycloudwave.connections.Connection.connect"
        ) as connect, mock.patch(
            "pycloudwave.connections.ssl.create_default_context",
...
class TestEscape(base.PyCloudwaveTestCase):
    def test_escape_string(self):
        ...
        # cloudwave-1.0 removes the option however
        if self.cloudwave_server_is(con, (8, 0, 0)):
    ...
    def test_escape_fallback_encoder(self):
        ...
        mapping = {str: pycloudwave.converters.escape_string}
