...
from pycloudwave.err import ProgrammingError
...
class TestCursor(base.PyCloudwaveTestCase):
    ...
    #    # result is from CloudwaveDB module
    ...
    #    conn = self.connect()
    #    c = conn.cursor()
    #    c.execute("select * from cloudwave.user")
    #
    ...
    def test_json(self):
        ...
        conn = pycloudwave.connect(**args)
        # MariaDB only has limited JSON support, stores data as longtext
        # https://mariadb.com/kb/en/json-data-type/
        if not self.cloudwave_server_is(conn, (5, 7, 0)):
            pytest.skip("JSON type is only supported on Cloudwave >= 3.1")
...
class TestBulkInserts(base.PyCloudwaveTestCase):
    cursor_type = pycloudwave.cursors.DictCursor
