...
import pycloudwave
from pycloudwave import cursors
from pycloudwave.tests import base

__all__ = ["TestOldIssues", "TestNewIssues", "TestGitHubIssues"]


class TestOldIssues(base.PyCloudwaveTestCase):
    ...
    def test_issue_6(self):
        ...
        # ToDo: this test requires access to db 'cloudwave'.
        kwargs = self.databases[0].copy()
        kwargs["database"] = "cloudwave"
        conn = pycloudwave.connect(**kwargs)
        c = conn.cursor()
...
KEY (`station`,`dh`,`echeance`)) ENGINE=MyISAM DEFAULT CHARSET=latin1;"""
    ...
    @pytest.mark.skip(
        "test_issue_17() requires a custom, legacy Cloudwave configuration and will not be run."
    )
    def test_issue_17(self):
        """could not connect cloudwave use password"""
        conn = self.connect()
        ...
        try:
            ...
            conn2 = pycloudwave.connect(host=host, user="issue17user", passwd="1234", db=db)
            c2 = conn2.cursor()
...
class TestNewIssues(base.PyCloudwaveTestCase):
    def test_issue_34(self):
        try:
            pycloudwave.connect(host="localhost", port=1237, user="root")
            self.fail()
    ...
    def test_issue_33(self):
        conn = pycloudwave.connect(charset="utf8", **self.databases[0])
    ...
    def test_issue_35(self):
        conn = self.connect()
        c = conn.cursor()
        print("sudo killall -9 cloudwaved within the next 10 seconds")
        try:
    ...
    def test_issue_38(self):
        conn = self.connect()
        c = conn.cursor()
        datum = "a" * 1024 * 1023  # reduced size for most default cloudwave installs
...
class TestGitHubIssues(base.PyCloudwaveTestCase):
    ...
    def test_issue_79(self):
        ...
        conn = self.connect()
        c = conn.cursor(pycloudwave.cursors.DictCursor)
    ...
    def test_issue_364(self):
        ...
        conn = pycloudwave.connect(charset="utf8mb4", **self.databases[0])
        self.safe_create_table(
            conn,
            "issue364",
        ...
        values = [pycloudwave.Binary(b"\x00\xff\x00"), "\xe4\xf6\xfc"]
